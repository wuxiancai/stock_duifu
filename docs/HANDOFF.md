...
- 新增 `POST /api/trade-plans/track-realtime`：先使用 AkShare Eastmoney/Sina 自动降级回补目标交易日计划股延迟实时行情，再返回计划跟踪结果和实时回补摘要。
