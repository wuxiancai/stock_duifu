...
- 2026-07-06 已加固 `/api/trade-plans/track-realtime failed: 502` 高频链路：API 默认实时行情源改为 `direct-sina`，只请求计划股/持仓股的新浪直连行情，不再默认退回 AkShare 全市场实时表；如确需旧自动后备，可显式设置 `STOCK_API_REALTIME_PROVIDER=auto`。接口新增全局非阻塞并发锁，已有实时回补正在执行时，后续请求直接返回 `provider=skipped_busy_realtime` 和中文跳过原因，并继续执行交易计划跟踪，避免自动刷新并发堆积导致 Vite 代理 502。验证：`.venv/bin/python -m pytest tests/test_trade_plan_generation.py tests/test_realtime_quote_workflow.py -q`：42 passed，1 个既有 LibreSSL/urllib3 warning。
