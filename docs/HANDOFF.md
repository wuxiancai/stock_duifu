...
- 当前已修复盘中跟踪“高开取消”误伤模拟撮合：高开超过买入上限 3% 不再无条件取消，只有全天/当前快照未回落触达买入区间时才取消追高；如果盘中低点已经触达买入区间，优先判定“已触发”并进入模拟买入。模拟撮合同步按该口径处理，避免京东方 A 这类盘中上涨且已回落触达买入区间的计划被错误标记“取消”。验证：`.venv/bin/python -m pytest tests/test_trade_plan_generation.py tests/test_simulation_trading.py tests/test_realtime_quote_workflow.py -q` 56 passed。
