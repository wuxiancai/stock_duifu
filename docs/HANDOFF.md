...
- `get_data.sh` 默认使用中国时区当天作为 `TRADE_DATE`，可显式传 `TRADE_DATE=YYYY-MM-DD` 或位置参数；也支持 `--start YYYYMMDD --end YYYYMMDD` 按日期区间运行盘后 workflow 和覆盖审计，但会先用 TuShare `trade_cal` 过滤为开市日；内部子脚本通过 `bash scripts/...` 调用，不依赖执行位。
