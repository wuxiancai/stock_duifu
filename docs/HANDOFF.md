...
- 当前已优化强势板块主榜口径：主榜改为“强势行业”，只使用 TuShare/DC 的 `idx_type=行业板块` 且 `level=东财一级行业`；概念题材保留为后续增强信号，不再与行业主榜混排。候选成分查询同步限定东财一级行业，前端文案已改为“强势行业/行业详情”，并说明概念题材不再与行业混排。验证：`.venv/bin/pytest tests/test_sector_provider.py tests/test_sector_ranking.py tests/test_candidate_provider.py tests/test_candidate_screening.py -q` 16 passed；`npm test -- --run` 3 passed；`npm run build` 通过。
