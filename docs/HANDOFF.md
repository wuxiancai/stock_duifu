...
- 2026-07-06 已补齐 `/api/trade-plans/track-realtime` 轻量多源备用链路：此前默认只用 `direct-sina`，若新浪直连失败会导致实时价无法写入，今日交易计划可能保持待触发、模拟交易无法买入。现已新增 `EastmoneyDirectRealtimeQuoteProvider`，默认 API/CLI 实时源改为 `auto` 轻量链路：`sina_direct_realtime -> eastmoney_direct_realtime`，均只按计划股/持仓股批量请求，不拉全市场；AkShare Eastmoney/Sina 全市场实时源保留为 `auto-full` 最后兜底，不进入默认路径。环境变量：默认不设置即可使用轻量双源；`STOCK_API_REALTIME_PROVIDER=auto-full` 才启用 AkShare 全市场最后兜底；`direct-sina` / `eastmoney` / `akshare` / `sina` 可单独指定。验证：`.venv/bin/python -m pytest tests/test_trade_plan_generation.py tests/test_realtime_quote_workflow.py -q`：44 passed，1 个既有 LibreSSL/urllib3 warning。
