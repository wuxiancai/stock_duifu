...
- 当前已加固 `get_data.sh` 交易日历过滤：区间模式、默认 bootstrap 模式会打印从 TuShare `trade_cal` 解析出的实际开市日列表；`run_one_date` 执行 workflow 前新增最后一道开市日校验，任何闭市日即使误混入日期集合，也会输出跳过并不执行 `run-after-close-workflow`。验证：`sh -n get_data.sh` 通过；`.venv/bin/python -m pytest tests/test_deployment_scripts.py -q` 16 passed。
