...
- 当前已优化云服务器部署体验：`deploy_ubuntu.sh` 会在 `.env` 缺少 `TUSHARE_TOKEN` 时交互询问，用户可直接回车跳过；会检查是否已有 PostgreSQL 数据卷，交互询问保留数据库或删除重建，非交互环境默认保留，显式 `RESET_DB=1` 可删除重建。部署脚本完成迁移、systemd 和 23 点 cron 安装后会自动执行 `bash start.sh`，随后自动检查 Web、API 和 `/api/system/database-health` 并打印结果。`start.sh` 启动时会检查数据库数据是否满足系统运行：不足则自动执行 `get_data.sh` 初始化/补齐；已有基础数据时会执行最新交易日增量检查；没有 token 时不会失败，会启动空页面并打印补 token/补数据命令。启动完成后会自动检测 Web 页面是否可访问，并打印局域网访问地址和防火墙命令。验证：`sh -n deploy_ubuntu.sh && sh -n start.sh && sh -n get_data.sh && sh -n stop.sh` 通过；`.venv/bin/python -m pytest tests/test_deployment_scripts.py -q` 16 passed。
