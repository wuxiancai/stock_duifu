...
- 当前已优化云服务器部署体验：`deploy_ubuntu.sh` 会在 `.env` 缺少 `TUSHARE_TOKEN` 时交互询问，用户可直接回车跳过；部署脚本完成迁移、systemd 和 23 点 cron 安装后会自动执行 `bash start.sh`。`start.sh` 在数据库缺数据但没有 token 时不会失败，会启动空页面并提示后续配置 token 和补数据命令。部署/启动完成说明已改为中文，包含访问地址、端口、日志、systemd、手动补数、重启和停止命令。验证：`sh -n deploy_ubuntu.sh && sh -n start.sh && sh -n get_data.sh && sh -n stop.sh` 通过；`STOCK_DEPLOY_DRY_RUN=1 FORCE_INSTALL=1 TUSHARE_TOKEN=token-for-dry-run ./deploy_ubuntu.sh` 通过；`.venv/bin/python -m pytest tests/test_deployment_scripts.py -q` 16 passed。
