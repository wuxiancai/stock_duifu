...
- [x] 已修正数据库健康监测的交易计划口径：当候选股票存在但股票池为空时，交易计划 0 条属于宁缺毋滥的正常空结果，不再误报 warning。
...
- [x] 已完成股票池与交易计划链路重构：候选 Top50 与股票池分层，股票池按核心主升 5 / 稳定强势 3 / 强势延续 2 生成，不足 10 只不补弱票；今日交易计划只对股票池全部生成；盘中触发只允许真实盘中实时轮询写入触发时间；模拟交易按触发时间顺序买入，资金不足记录“已触发但资金不足，虚拟交易已买入”；新增虚拟持仓和虚拟交易记录，不计入资金曲线。
- [x] 已修复盘中跟踪状态语义：盘中实时轮询未触发时保持待触发，不提前显示取消；只有收盘确认后仍未成交才标记取消。
