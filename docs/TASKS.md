...
目标：让 `PRD_MVP.md` 第 17 章在开发任务中逐项可追踪，避免模拟交易模块被前面任务 11-13 的基础实现覆盖后遗漏剩余规则。

#### 17.1.1 模块定位

状态：基础能力已完成，完整实战验证仍待目标交易日真实行情验收。

- 已完成：模拟交易不接券商 API、不使用真实资金；用于验证交易计划触发、持仓、市值、资金曲线、胜率、最大回撤等基础指标。
- 待补齐：盈亏比统计口径、分批止盈和移动止盈完整落地后，再形成更完整的实战绩效评估。

#### 17.1.2 与交易计划的关系

状态：已完成基础链路。

- 已完成：模拟交易只基于系统生成的 `trade_plan` 运行，不允许计划外交易。
- 已完成：`run_simulation_workflow` 会先跟踪交易计划触发状态，再基于已触发计划执行模拟交易。
- 待验收：在 `2026-06-22` 目标交易日当天用真实实时快照验证触发、模拟成交和持仓变化。
...
- 默认模拟账户初始资金为 `1000000`。
- 账户、持仓、交易和权益曲线按复利资产口径滚动记录。

#### 17.1.4 资金账户字段

状态：已完成基础字段。

- 已覆盖：账户 ID、账户名称、初始资金、总资产、可用现金、持仓市值、冻结资金、累计盈亏、累计收益率、最大回撤、创建时间、更新时间。
- 待继续观察：冻结资金当前仅作为账户字段保留，后续若引入挂单状态再补充实际冻结逻辑。

#### 17.1.5 交易费用规则

状态：部分完成。

- 已完成：买卖双边佣金、卖出印花税、买卖双边过户费和最低佣金已计入模拟交易。
- 待补齐：费率参数仍需从硬编码常量迁移到配置项，满足 PRD “费率变化只改配置，不改交易逻辑”的要求。

#### 17.1.6 买入规则

状态：基础买入规则已完成。

- 已完成：只买计划内股票；按建议仓位计算计划金额；按 100 股整数倍下单；检查现金和费用；可买数量小于 100 股不买入。
- 已完成：涨停无法买入、高开过多取消、低开破位取消、缺少止损价不买入等保守规则。
- 待补齐：强势买入中的 VWAP 触发、盘中量能细化和更接近实时撮合的成交价规则。

#### 17.1.7 卖出规则

状态：部分完成。

- 已完成：跌破止损价全卖；跌停无法卖出时不强行成交；卖出费用计入账户。
- 待补齐：第一止盈卖出 50%、第二止盈再卖 30%、跌破 MA5 卖出剩余仓位、市场转风险降仓、板块退潮降仓、持仓超期处理和快速跳水滑点记录。

#### 17.1.8 持仓管理

状态：已完成基础字段。

- 已覆盖：账户、交易计划、股票、板块、策略、买入均价、当前价、数量、市值、成本、浮盈浮亏、浮动收益率、止损价、止盈价、持仓状态、买入原因、卖出原因和时间字段。

#### 17.1.9 交易记录

状态：已完成基础字段。

- 已覆盖：账户、交易计划、股票、交易日期、交易方向、成交价、成交数量、成交金额、费用、净发生金额、盈亏、收益率、原因和创建时间。
- 待补齐：如果后续需要盘中多次撮合，应显式展示和验证交易时间字段。

#### 17.1.10 买入原因和卖出原因

状态：已完成基础记录。

- 已完成：买入和卖出都会记录原因，当前重点覆盖买入触发、止损卖出和保守不成交原因。
- 待补齐：分批止盈、MA5 移动止盈、板块退潮、市场风险和持仓超期对应的标准化原因。

#### 17.1.11 模拟交易页面

状态：部分完成。

- 已完成：Web 工作台已有模拟交易展示和“跟踪并模拟交易”操作。
- 待补齐：若前端正式引入路由，需要提供 PRD 指定的独立 `/simulation` 页面。

#### 17.1.12 模拟交易页面内容

状态：大部分完成。

- 已完成：账户概览、资金曲线、当前持仓、交易记录、风险指标和运行入口。
- 待补齐：完整的按日期筛选、分批止盈状态和更多绩效统计维度。

#### 17.1.13 账户概览字段

状态：大部分完成。

- 已覆盖：初始资金、总资产、可用现金、持仓市值、累计盈亏、累计收益率、最大回撤。
- 待补齐：当日盈亏、今日收益率和持仓股票数需要在真实目标日模拟成交后继续验收页面展示口径。

#### 17.1.14 今日持仓表

状态：大部分完成。

- 已覆盖：股票、板块、策略、买入价、当前价、数量、市值、浮盈浮亏、止损价、止盈价和持仓状态。
- 待补齐：持仓天数、第一止盈/第二止盈执行状态、计划卖出条件和待卖出状态。

#### 17.1.15 今日交易记录表

状态：部分完成。

- 已覆盖：买入/卖出、股票、成交价、成交数量、成交金额、手续费、盈亏和交易原因。
- 待补齐：交易时间、仓位变化后比例、买入/卖出后现金等更细展示字段。

#### 17.1.16 模拟交易定时任务

状态：部分完成。

- 已完成：手动 CLI、脚本和 API workflow，可按目标交易日执行实时快照回补、计划跟踪和模拟交易。
- 待补齐：交易日 `09:30-15:00` 每 1-5 分钟自动轮询的定时任务；定时任务必须继续遵守真实数据和日期保护，不伪造行情。

#### 17.1.17 模拟交易成功标准

状态：部分完成。

- 已完成：默认 100 万初始资金、复利账户记录、基于交易计划自动模拟买入基础链路、止损卖出基础链路、费用计算、持仓记录、交易记录、买卖原因、资金/市值/收益/最大回撤展示。
- 待补齐：分批止盈、移动止盈、盈亏比统计和目标交易日真实模拟成交证据。

#### 17.1.18 模拟交易设计原则

状态：大部分完成，仍需真实目标日继续验收。

- 已完成：保守成交、涨停不买、跌停不卖、高开过多不追、低开破位取消、扣除交易费用、100 股整数倍、现金不足不买、没有止损价不买、只执行计划内股票。
- 待验收：真实盘中/延迟行情写入后，确认这些原则在目标交易日仍按预期生效。

#### 17.1.19 与 MVP 的关系

状态：已纳入 MVP 后续验证链路，不得宣称第 17 章全部完成。

- 已完成：任务 11-13 已把模拟交易作为 MVP 后续增强接入账户、持仓、交易记录、资金曲线、API、CLI、Web 和实时 workflow。
- 下一步：先完成 `2026-06-22` 目标交易日真实实时快照写入与模拟成交验收，再补分批止盈、移动止盈、费率配置和交易时段轮询。
