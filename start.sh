...
latest_open_data_is_complete() {
  local status_line="$1"
  local latest_open stock_daily_rows market_rows sector_rows candidate_rows current_candidate_rows stock_pool_rows plan_rows data_jobs
  if [ -z "$status_line" ]; then
    return 1
  fi
  IFS='|' read -r latest_open stock_daily_rows market_rows sector_rows candidate_rows current_candidate_rows stock_pool_rows plan_rows data_jobs <<<"$status_line"
  if [ -z "$latest_open" ]; then
    return 1
  fi
  [ "$stock_daily_rows" -gt 0 ] && [ "$market_rows" -gt 0 ] && [ "$sector_rows" -gt 0 ] && [ "$candidate_rows" -gt 0 ] && [ "$current_candidate_rows" -gt 0 ] && [ "$stock_pool_rows" -gt 0 ] && [ "$plan_rows" -gt 0 ] && [ "$data_jobs" -gt 0 ]
}
